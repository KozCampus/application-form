...
class APISettings(Struct):
    app_name: str = "Litestar API"
    """
    Application name displayed e.g. in the OpenAPI documentation.
    """

    debug: bool = os.environ.get("APP_DEBUG", "True") in TRUE_VALUES
    """
    ...
    allow_origins: list[str] = msgspec.field(
        default_factory=lambda: (
            [
                origin.strip() 
                for origin in os.environ.get("APP_ALLOW_ORIGINS", "").split(",") 
                if origin.strip()
            ]
            if os.environ.get("APP_ALLOW_ORIGINS")
    ...
    redirect_url: str = os.environ.get(
        "APP_REDIRECT_URL",
        "http://localhost:5173",
    )
    """
    ...
    cache_expiration: int = int(os.environ.get("APP_CACHE_EXPIRATION", 60))
    """
...
class SSOProviderSettings(Struct):
    client_id: str = os.environ.get("APP_GOOGLE_CLIENT_ID", "")
    """
    OAuth2 client ID assigned by the provider.
    """

    client_secret: str = os.environ.get("APP_GOOGLE_CLIENT_SECRET", "")
    """
